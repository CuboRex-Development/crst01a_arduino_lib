...
// データ定期送信周期設定(0x40)
// 送信周期の設定を行う電文です。
// 引数：frequency：0x00：10Hz, 0x01：20Hz, 0x02：50Hz
// 戻り値：なし
void Crst01a::SetCycleReqFrequency(uint8_t frequency){
	l_reqCycleMsg.data[0] = frequency;
}

// データ定期送信設定(0x40)(設定)
// 各電文の定期送信の切り替えを行う電文です。
...
// data idのインクリメント
// l_dataIdをインクリメントして結果を返す
// 引数：なし
// 戻り値：インクリメント後のl_dataId
