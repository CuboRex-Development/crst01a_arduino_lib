...
#define DEF_SPEED		(0.1)		// 未設定時のデフォルトスピード
...
class CuGoDiffDriveCtrl{
	...
	public:
		...
		bool MoveForward(float targetDistance, float targetSpeed = DEF_SPEED);					// 前後進制御
		bool MoveTurn(float targetDegree, float targetSpeed = DEF_SPEED);						// 旋回制御
		bool MoveCurve(float targetRadius, float targetDegree, float targetSpeed = DEF_SPEED);	// 円軌道での移動命令
		bool Stop(void);																		// 停止命令
